...
namespace MarketDataMonitorAPI
{
     public class BuyShares
     {
        ...
        public async Task ExecuteOrder(string ticker, int shares)
        {
            ...
            var order = await client.PostOrderAsync(ticker, shares, OrderSide.Buy, OrderType.Market, TimeInForce.Day);

            // Submit a limit order to attempt to sell 1 share of AMD at a
            // particular price ($20.50) when the market opens
            //order = await client.PostOrderAsync(ticker, shares, OrderSide.Sell, OrderType.Market, TimeInForce.Day);
